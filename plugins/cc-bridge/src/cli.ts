...
import { translate, type OutputFile } from "./adapters/codex.js";
...
import { readSkills, type SkillRecord } from "./readers/skills.js";
...
const GAP_MARKERS = ["skipped", "no Codex equivalent"] as const;
...
interface ReaderBundle {
  skills: SkillRecord[];
  agents: ReturnType<typeof readAgents>;
  hooks: ReturnType<typeof readHooks>;
  env: ReturnType<typeof readEnv>;
  context: ReturnType<typeof readContext>;
}

function runReaders(projectRoot: string): ReaderBundle {
  return {
...
function runTranslate(target: string, inputs: ReaderBundle, projectRoot: string): OutputFile[] {
  if (target !== "codex") {
...
function classifyWarnings(warnings: string[]): { gaps: string[]; actions: string[] } {
...
function collectExtraFileCopies(skills: SkillRecord[], projectRoot: string): Array<{ src: string; dst: string }> {
...
function writeOutput(projectRoot: string, relPath: string, content: string): "created" | "updated" {
  const dst = path.join(projectRoot, relPath);
  const action: "created" | "updated" = fs.existsSync(dst) ? "updated" : "created";
  fs.mkdirSync(path.dirname(dst), { recursive: true });
  fs.writeFileSync(dst, content, "utf-8");
  return action;
}

function copyExtra(projectRoot: string, src: string, relDst: string): "created" | "updated" {
...
function scanOrphanCandidates(projectRoot: string): string[] {
...
function walkFiles(dir: string, onFile: (abs: string) => void): void {
...
function walkForAgents(dir: string, root: string, out: Set<string>): void {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      if (ORPHAN_SCAN_SKIP_DIRS.has(entry.name)) continue;
...
interface PreparedOutputs {
  outputs: OutputFile[];
  extras: Array<{ src: string; dst: string }>;
  warnings: string[];
}

function prepare(target: string, projectRoot: string): PreparedOutputs {
  ...
  for (const out of outputs) warnings.push(...(out.warnings ?? []));
  return { outputs, extras, warnings };
}

export interface SyncReport {
  synced: Array<{ path: string; action: string }>;
  warnings: string[];
  gaps: string[];
  actions_required: string[];
}

export interface CmdSyncOpts {
  dryRun: boolean;
  hookPlanPath?: string | null;
...
export function cmdSync(
  target: string,
  projectRoot: string,
  opts: CmdSyncOpts = { dryRun: false },
): SyncReport {
  const { outputs, extras, warnings } = prepare(target, projectRoot);
  const synced: Array<{ path: string; action: string }> = [];

  const hookOutputs: OutputFile[] = [];
  ...
  if (opts.hookPlanPath) {
    ...
    for (const out of planOutputs) effectiveWarnings.push(...(out.warnings ?? []));
  } else {
  ...
  const allOutputs: OutputFile[] = [...outputs, ...hookOutputs];
...
export function cmdDiff(target: string, projectRoot: string): DiffReport {
  const { outputs, extras, warnings } = prepare(target, projectRoot);
  const diffs: Array<{ path: string; diff: string }> = [];

  for (const out of outputs) {
    if (out.path === HOOKS_OUTPUT_PATH) continue;
    const rel = out.path;
    const dst = path.join(projectRoot, rel);
    const existing = readOrNull(dst) ?? "";
...
export function cmdStatus(target: string, projectRoot: string): StatusReport {
  ...
  for (const { src, dst: relDst } of extras) {
    ...
    const dst = path.join(projectRoot, relDst);
    if (!isFile(dst)) {
      if (isFile(src)) missing.push(rel);
      continue;
    }
    if (!isFile(src)) continue;
    let existing: string;
...
export interface CmdHooksInventoryOpts {
  userHome?: string | null;
  externalProjectPath?: string | null;
}

export function cmdHooksInventory(
  projectRoot: string,
  opts: CmdHooksInventoryOpts = {},
): InventoryReport {
  ...
  return {
    entries: entries.map((e) => ({
      ...
      event: e.event,
      matcher: e.matcher,
      handler: e.handler as Record<string, unknown>,
      label: e.label,
    })),
  };
}

function countHookHandlers(hooks: ReturnType<typeof readHooks>): number {
  let n = 0;
...
interface ParsedArgs {
  command: "sync" | "diff" | "status" | "hooks-inventory";
  target: string;
  projectRoot: string;
  hookPlanPath: string | null;
  externalProjectPath: string | null;
...
function parseArgs(argv: string[]): ParsedArgs {
  ...
  let hookPlanPath: string | null = null;
  let externalProjectPath: string | null = null;
  let dryRun = false;
  ...
  if (!projectRoot) throw new UsageError("--project-root is required");
...
function usage(): string {
  return [
    "usage: cc-bridge <command> --target codex --project-root PATH [options]",
    "",
    "commands:",
