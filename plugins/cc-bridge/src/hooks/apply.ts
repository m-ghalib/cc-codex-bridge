...
import type { OutputFile } from "../adapters/codex.js";
import { HOOK_EVENTS_MAP, HOOK_HANDLER_TYPES_MAP } from "../mappings.js";
import type { HookHandler } from "../readers/hooks.js";
import { ensureCodexHooksEnabled, type EnablementResult } from "./enablement.js";
...
export interface ApplyHookPlanOpts {
  plan: HookPlan;
...
export function applyHookPlan(opts: ApplyHookPlanOpts): OutputFile[] {
  ...
  let composed: Record<string, CodexHookEntry[]>;
  switch (opts.plan.write_mode) {
    ...
    case "create":
      if (existing && Object.keys(existing).length > 0) {
        warnings.push(
          `Hook plan write_mode "create": ${HOOKS_OUTPUT_PATH} already has entries; existing content will be replaced`,
        );
      }
      composed = composeFromTranslated(translated);
      break;
  ...
  if (opts.plan.enable_hooks && opts.plan.enable_scope) {
    if (opts.dryRun) {
      warnings.push(
        `Codex hooks enablement deferred (dry run): would set [features] codex_hooks = true in ${enablementHint(opts.plan.enable_scope, opts.projectRoot, opts.userHome)}`,
      );
    } else {
...
function translateEntry(event: string, _matcher: string, handler: HookHandler): {
  codexEvent: string | null;
  ...
  if (!mappedType) {
    warnings.push(
      `Hook event '${event}': handler type '${String(htype)}' not supported in Codex; skipped`,
    );
    return { codexEvent, handler: null, warnings };
  }

  const newHandler: Record<string, unknown> = { type: mappedType };
...
function readExistingHooks(hooksPath: string): Record<string, CodexHookEntry[]> | null {
  ...
  try {
    ...
    throw new HookPlanError(
      `Existing ${HOOKS_OUTPUT_PATH} is unreadable: ${(err as Error).message}. ` +
  ...
  for (const [event, entries] of Object.entries(hooks as Record<string, unknown>)) {
    ...
    for (const entry of entries) {
      ...
      const filteredHandlers: Array<Record<string, unknown>> = [];
      for (const h of handlers) {
        if (isPlainObject(h)) filteredHandlers.push(h as Record<string, unknown>);
      }
...
function composeFromTranslated(translated: TranslatedEntry[]): Record<string, CodexHookEntry[]> {
  const out: Record<string, CodexHookEntry[]> = {};
...
function mergeHooks(
  existing: Record<string, CodexHookEntry[]> | null,
...
function enablementHint(scope: "project" | "user", projectRoot: string, userHome: string | null | undefined): string {
  if (scope === "project") return path.join(projectRoot, ".codex", "config.toml");
  const home = userHome ?? process.env.HOME ?? "~";
  return path.join(home, ".codex", "config.toml");
}

function formatEnablementWarning(result: EnablementResult, scope: "project" | "user"): string {
  if (result.changed) {
