...
const ARGS_WARNING_MARKER = "<!-- ARGUMENTS placeholder not supported in Codex; substitute manually -->";
const EXEC_WARNING_MARKER = "<!-- Inline `!` exec block not supported in Codex; run command manually -->";
function escapeRegExp(s) {
    return s.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
}
function rewriteToolRefs(text, tools) {
    if (!text)
        return text;
    ...
    for (const claudeName of names) {
        ...
        out = out.replace(new RegExp(`\\bthe ${escaped} tool\\b`, "g"), `the ${codexName} tool`);
        out = out.replace(new RegExp(`\\bUse ${escaped}\\b`, "g"), `Use ${codexName}`);
    }
    return out;
}
function stripSkillSyntax(body) {
    const warnings = [];
...
function composeDescription(description, whenToUse) {
    const parts = [];
...
function translateSkill(skill, tools) {
    const warnings = [];
    ...
    const newFm = { name: skill.name || skillId };
...
function globPathPrefix(globPath) {
...
function resolveScopePrefix(rawPath, projectRoot) {
...
function translateContext(context, tools, projectRoot) {
    const outputs = [];
    const rootParts = [];
...
export function translate(skills, agents, env, context, projectRoot) {
    const tools = TOOLS_MAP;
    const outputs = [];
