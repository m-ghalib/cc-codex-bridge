# cc-codex-bridge: Claude Code to Codex CLI Configuration Bridge

**Date:** 2026-04-22
**Status:** Draft (historical)
**Repo:** `~/GitHub/cc-codex-bridge` (new, fresh repo with selective copy from product-os)
...
cc-codex-bridge is a Claude Code plugin that reads Claude Code configuration (skills, subagents, hooks, env vars, context files, and rules) and translates it into Codex CLI-native configuration files. Claude Code is the source of truth; sync is unidirectional (Claude Code -> Codex). Gemini CLI support is deferred to v2.
...
| Name | `cc-codex-bridge` | "Claude Code bridge" — concise, clear origin |
...
| Doc refresh | Repo-local source registry in `cc-codex-bridge` | Keeps platform knowledge current without external adapter dependencies |
...
├── .github/
│   └── workflows/
│       ├── cc-codex-bridge-pr-check.yml   # required PR gate
...
|---|---|---|
| `cc-codex-bridge-pr-check.yml` | Required PR gate for the repo | `pull_request` on `opened`, `synchronize`, `reopened`, `ready_for_review` |
...
- Configure `main` so `cc-codex-bridge-pr-check` is a required status check before merge.
...
| `.github/workflows/cli-refresh.yml` | `.github/workflows/cli-refresh.yml` | Adapt for cc-codex-bridge repo |
...
{
  "enabledPlugins": {
    "cc-codex-bridge@file:///Users/<you>/GitHub/cc-codex-bridge": true
  }
}
```

Future: publish to a marketplace for `cc-codex-bridge@<marketplace>` install.
