...
export const EVENT_DISPLAY: Readonly<Record<string, string>> = {
...
const SCOPE_DISPLAY: Readonly<Record<HookScope, string>> = {
  user: "User",
  project: "Project",
};

export function summarizeHandler(handler: HookHandler): string {
...
function commandSummary(command: string): string {
  ...
  if (!firstToken) return trimmed.slice(0, 32);
...
function promptSummary(prompt: string): string {
  ...
  if (phrase.length === 0) return cleaned.slice(0, 48);
  if (phrase.length <= 48) return phrase;
  return phrase.slice(0, 48).trimEnd() + "…";
}

export function buildLabel(scope: HookScope, event: string, handler: HookHandler): string {
